const Icons = {
  ...
  camSettings:(c,s=18)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>,
...
function makeCSS(T) {
  return `@import url('https://fonts.googleapis.com/css2?family=M+PLUS+Rounded+1c:wght@300;400;500;700&family=Outfit:wght@300;400;600;700&display=swap'); *{box-sizing:border-box;margin:0;padding:0;} input:not(.ob-input):not(.ob-num),select,textarea{font-size:16px !important;} body{overflow:hidden;background:${T.bg};} .app{width:100%;height:100%;display:flex;flex-direction:column;background:${T.bg};color:${T.text};font-family:'M PLUS Rounded 1c',sans-serif;font-size:14px;overflow-x:hidden;} .hdr{padding:10px 14px 8px;padding-top:calc(10px + env(safe-area-inset-top,0px));background:${T.bg};border-bottom:1px solid ${T.soft};display:flex;align-items:center;justify-content:space-between;flex-shrink:0;} .htitle{font-family:'M PLUS Rounded 1c',sans-serif;font-size:15px;font-weight:700;color:${T.primary};} .hsub{font-size:8px;color:${T.accent}88;} .ham{width:32px;height:32px;display:flex;align-items:center;justify-content:center;cursor:pointer;background:${T.soft};border:none;border-radius:8px;} .content{flex:1;overflow-y:auto;overflow-x:hidden;} .nav{display:flex;background:${T.card};border-top:1px solid ${T.soft};padding:0;padding-bottom:env(safe-area-inset-bottom,0px);flex-shrink:0;} .nb{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;gap:4px;padding:14px 1px 8px;border:none;background:transparent;cursor:pointer;border-radius:7px;color:${T.soft};transition:all .2s;min-height:80px;} .nb.on{color:${T.primary};background:transparent;transform:scale(1.15);} .nb-lbl{font-size:11px;font-family:'M PLUS Rounded 1c',sans-serif;font-weight:600;} .card{background:${T.card};border-radius:14px;padding:13px;box-shadow:0 2px 8px ${T.primary}0d;} .ct{font-size:10px;font-weight:700;color:${T.accent};margin-bottom:10px;letter-spacing:.5px;} .btn{padding:9px 16px;border:none;border-radius:12px;font-size:13px;font-weight:600;cursor:pointer;font-family:'M PLUS Rounded 1c',sans-serif;transition:all .15s;display:inline-flex;align-items:center;justify-content:center;gap:4px;} .bp{background:${T.primary};color:#fff;} .bs{background:${T.soft};color:${T.primary};} .bsm{padding:5px 11px;font-size:11px;border-radius:8px;} .blg{padding:13px 20px;font-size:15px;border-radius:13px;} input,select,textarea{width:100%;padding:8px 12px;border:1.5px solid ${T.soft};border-radius:10px;background:${T.bg};color:${T.text};font-size:13px;font-family:'M PLUS Rounded 1c',sans-serif;outline:none;box-sizing:border-box;} select{text-align:center;text-align-last:center;} input:focus,select:focus,textarea:focus{border-color:${T.primary};} label{font-size:11px;font-weight:600;color:${T.accent};margin-bottom:3px;display:block;} .mo{position:fixed;inset:0;background:#00000077;z-index:200;display:flex;align-items:flex-end;justify-content:center;} .md{background:${T.card};border-radius:20px 20px 0 0;padding:20px;width:100%;max-width:100%;max-height:90dvh;overflow-y:auto;color:${T.text};box-sizing:border-box;} .md label{color:${T.accent};} .md input,.md select,.md textarea{background:${T.bg};color:${T.text};border-color:${T.soft};} .mdtitle{font-family:'M PLUS Rounded 1c',sans-serif;font-size:16px;font-weight:700;color:${T.primary};margin-bottom:14px;} .overlay{position:fixed;inset:0;background:#00000055;z-index:299;} .drawer{position:fixed;top:0;left:0;bottom:0;width:80%;max-width:300px;background:${T.card};z-index:300;transform:translateX(-100%);transition:transform .26s cubic-bezier(.4,0,.2,1);overflow-y:auto;box-shadow:4px 0 20px #00000033;color:${T.text};} .drawer.open{transform:translateX(0);} .di{display:flex;align-items:center;gap:12px;padding:11px 14px;cursor:pointer;border-radius:9px;margin:2px 7px;transition:background .12s;} .di:hover{background:${T.soft};} .di-lbl{font-size:16px;font-weight:500;color:${T.text};font-family:'M PLUS Rounded 1c',sans-serif;} .sep{height:1px;background:${T.soft};margin:7px 14px;} .trow{display:flex;align-items:center;justify-content:space-between;padding:9px 0;border-bottom:1px solid ${T.soft}22;} .trow span{color:${T.text};} .tg{width:44px;height:24px;border-radius:12px;position:relative;cursor:pointer;transition:background .2s;border:none;flex-shrink:0;} .tg-k{width:20px;height:20px;border-radius:50%;background:#fff;position:absolute;top:2px;transition:left .2s;box-shadow:0 1px 4px #0003;} .wr{display:flex;align-items:center;justify-content:space-between;padding:7px 0;border-bottom:1px solid ${T.soft}33;} .pgrid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;} .pi{border-radius:11px;overflow:hidden;position:relative;background:${T.soft};aspect-ratio:1/1;} .pi img{width:100%;height:100%;object-fit:cover;} .bar-wrap{display:flex;align-items:flex-end;gap:2px;height:200px;} .bar-col{flex:1;display:flex;flex-direction:column;align-items:center;gap:2px;cursor:pointer;min-width:0;} .bar-f{width:100%;border-radius:3px 3px 0 0;min-height:2px;transition:height .4s;} .bar-l{font-size:7px;overflow:hidden;white-space:pre-line;width:100%;text-align:center;line-height:1.3;} .pin-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin:12px 0;} .pin-btn{padding:0;height:56px;border:none;border-radius:11px;background:${T.soft};color:${T.text};font-size:18px;font-weight:600;cursor:pointer;font-family:'M PLUS Rounded 1c',sans-serif;display:flex;align-items:center;justify-content:center;} .pin-dots{display:flex;gap:10px;justify-content:center;margin:9px 0;} .pin-dot{width:11px;height:11px;border-radius:50%;border:2px solid ${T.primary};} .pin-dot.on{background:${T.primary};} .sess{padding:7px 11px;background:${T.soft};border-radius:9px;margin-bottom:4px;font-size:11px;display:flex;justify-content:space-between;align-items:center;gap:6px;color:${T.text};cursor:pointer;} .reason-chip{padding:6px 10px;border:none;border-radius:20px;font-size:11px;font-weight:600;cursor:pointer;font-family:'M PLUS Rounded 1c',sans-serif;background:${T.soft};color:${T.primary};white-space:nowrap;flex-shrink:0;} .stats-tab{flex:1;padding:7px 4px;border:none;border-radius:8px;font-size:11px;font-weight:600;cursor:pointer;font-family:'M PLUS Rounded 1c',sans-serif;background:transparent;color:${T.text}66;transition:all .15s;} .stats-tab.on{background:${T.primary};color:#fff;} .cal-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:1px;} .cal-cell{min-height:44px;display:flex;flex-direction:column;align-items:center;cursor:pointer;border-radius:5px;padding:2px 1px;position:relative;} input[type='time']::-webkit-calendar-picker-indicator{display:none;width:0;padding:0;margin:0;} input[type='time']{-webkit-appearance:none;} input[type='date'].ob-input::-webkit-date-and-time-value{text-align:center;} input[type='date'].ob-input::-webkit-inner-spin-button{display:none;} input[type='date'].ob-input::-webkit-calendar-picker-indicator{opacity:0;position:absolute;width:100%;height:100%;left:0;top:0;cursor:pointer;} input.ob-input{border:none!important;border-bottom:3px solid!important;border-radius:0!important;background:transparent!important;padding:8px 0!important;width:100%!important;box-shadow:none!important;} input.ob-num{border:none!important;border-bottom:2px solid!important;border-radius:0!important;background:transparent!important;box-shadow:none!important;padding:0 0 4px!important;font-size:55px!important;width:140px!important;} input[type='range']::-webkit-slider-thumb{-webkit-appearance:none;width:28px;height:28px;border-radius:50%;background:#fff;border:2.5px solid ${T.primary};box-shadow:0 2px 10px ${T.primary}44;cursor:grab;transition:transform .15s,box-shadow .15s;} input[type='range']::-webkit-slider-thumb:active{transform:scale(1.3);box-shadow:0 2px 18px ${T.primary}66;cursor:grabbing;}`;
}
  ...
  return(
    <>
      ...
      <div className={`drawer${open?" open":""}`}>
        <div style={{padding:"18px 14px 10px",paddingTop:"calc(25px + env(safe-area-inset-top,0px))",borderBottom:`1px solid ${T.soft}`}}>
        ...
        <div style={{padding:"8px 0"}}>
          ...
          <div className="di" onClick={()=>{window.open(AFFILIATE_ITEMS[0].url,"_blank");onClose();}}>
            ...
            <span className="di-lbl">{t("affiliateTitle")}</span>
          </div>
          ...
          <div className="di" onClick={()=>{onReset();onClose();}}>
            ...
            <span className="di-lbl" style={{color:"#E74C3C"}}>{t("reset")}</span>
          </div>
        </div>
      </div>
    </>
    ...
    <div className="mo" onClick={onClose}>
      <div className="md" onClick={e=>e.stopPropagation()}>
        <div style={{textAlign:"center",marginBottom:12}}>
          ...
          <div style={{fontFamily:"'M PLUS Rounded 1c',sans-serif",fontSize:15,fontWeight:700,color:T.primary,marginBottom:4}}>{title}</div>
          <div style={{fontSize:12,color:T.text+"88",marginBottom:12}}>{body}</div>
        </div>
